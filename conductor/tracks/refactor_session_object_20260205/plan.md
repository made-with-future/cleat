...
- [ ] Task: Create `internal/session` package
    - [ ] Define `Session` struct in `internal/session/session.go`
    - [ ] Add constructor `NewSession(cfg *schema.Config, exec executor.Executor)`
